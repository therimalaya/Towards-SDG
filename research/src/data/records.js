...
const Records = [
  ...
  },
  {
    ...
    Research: {
      ...
      Type: "Experiments",
      URL:
        "https://www.sciencedirect.com/science/article/abs/pii/S0305750X19304954?CMX_ID=&SIS_ID=0&dgcid=STMJ_75273_AUTH_SERV_PPUB&utm_acid=13640654&utm_campaign=STMJ_75273_AUTH_SERV_PPUB&utm_dgroup=Email1Publishing&utm_in=DM636629&utm_medium=email&utm_source=AC_30&utm_term=Email%201%20Publishing_TLSH"
    },
    SDGRecords: [
      {
        Goals: [13, 15],
...
const parseEdges = record =>
  record.SDGRecords.filter(item => item.Targets.length > 1).map(item => ({
    source: String(item.Targets[0]),
    target: String(item.Targets[1]),
    style: {
      stroke:
    ...
    // dashes: item.Interaction.type === "Direct" ? true : false,
  }));
