...
import { Box, Button, Grid, List, ListItem, ListItemAvatar, ListItemText, Avatar, Typography, Link, Card, CardMedia, CardContent } from '@material-ui/core';
...
function SummaryText(props) {
  ...
  return (
    <ListItem>
      ...
      <ListItemText primary=<SummarySubText name={name} value={value}/>/>
      </ListItem>
  );
};
...
export default function Summary(prop) {
  ...
  const SelectedGoals = states.Goals.filter(goal=>goal.isSelected).map(goal=>{
    goal.targets = goal.targets.filter(target=>target.isSelected)
    return goal;
  })
      ...
      <Grid container spacing={3}>
        <Grid item sm={12} md={6}>
          <List className={classes.root}>
            ...
            <SummaryText name="Coauthor Faculties" value={states.Coauthors.Faculty.map((faculty, idx) => {
              return (
                <Typography className={classes.summaryText} key={idx}>
                  {Faculties[faculty]}
                </Typography>
              );
        ...
        <Grid item sm={12} md={6}>
          ...
          <Button
            ...
            color="primary"
            onClick={Submit(data)}>
            <Typography variant="button" className={classes.btnText}>Submit</Typography>
            <Send />
          </Button>
        </Grid>
      </Grid>
